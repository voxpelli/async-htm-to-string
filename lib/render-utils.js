...
import { assertTypeIsNever, explainVariable, isType, typesafeIsArray } from '@voxpelli/typed-utils';
...
async function * renderStringItem (item) {
  ...
  if (Object.hasOwn(omittedCloseTags, tag)) {
    yield `<${tag}${renderPropsSync(props)} />`;
...
async function * renderElement (item) {
  ...
  if (type === undefined) {
    ...
    const isThenable = !isPromise(rawResult) && typeof rawResult === 'object' && rawResult !== null && 'then' in rawResult && typeof rawResult.then === 'function';
    const result = isPromise(rawResult) || isThenable ? await rawResult : rawResult;
    ...
    if (!skipStringEscape) {
      if (typeof result === 'object' && result !== null && !Array.isArray(result) && result.async === false) {
        ...
        yield * typesafeIsArray(result)
          ? renderIterable(/** @type {RenderableElement[]} */ (result))
          : renderItem(result);
      }
    ...
    /* c8 ignore next -- exhaustiveness guard: type is string | function */
    assertTypeIsNever(type, `Invalid element type: ${explainVariable(type)}`);
  }
}
...
export async function * renderItem (rawItem) {
  ...
  if (item === undefined || item === null) {
    ...
    assertTypeIsNever(item, `Invalid render item type: ${explainVariable(item)}`);
  }
}

// *** SYNC RENDER FUNCTIONS ***

export class AsyncFallbackError extends Error {
  constructor () {
    super('Sync render encountered async content, falling back to async path');
...
/** @type {(item: BasicRenderableElement<ElementProps>) => string} */
function renderElementSync (item) {
  ...
  if (type === undefined) {
    ...
    if (isPromise(result) || (typeof result === 'object' && result !== null && 'then' in result && typeof result.then === 'function')) {
...
/** @type {(rawItem: RenderableElement|RenderableElement[]) => string} */
export function renderItemSync (rawItem) {
  ...
  if (item === undefined || item === null) {
    ...
    return renderChildrenSync(item);
  } else if (isAsyncIterable(item)) {
    throw new AsyncFallbackError();
  } else if (isIterable(item)) {
    return renderChildrenSync([.../** @type {Iterable<RenderableElement>} */(item)]);
  } else if (typeof item === 'object') {
