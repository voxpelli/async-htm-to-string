...
const ASYNC_FALLBACK = 'ASYNC_FALLBACK';
...
function renderPropsSync (props) {
  ...
  for (const propKey in props) {
    ...
    if (propValue === true) {
      ...
      result += ` ${propKey}="${stringifyEntities(propValue, { escapeOnly: true, useNamedReferences: true })}"`;
    } else if (typeof propValue === 'number') {
...
function renderElementSync (item) {
  ...
  if (type === undefined) {
    ...
    if (isPromise(result)) {
      throw new Error(ASYNC_FALLBACK);
    }

    if (!skipStringEscape) {
      return renderItemSync(result);
...
export function renderItemSync (rawItem) {
  ...
  if (item === undefined || item === null) {
    ...
    return stringifyEntities(item, { escapeOnly: true, useNamedReferences: true });
  } else if (typeof item === 'number') {
    return item + '';
  } else if (Array.isArray(item)) {
    return item.map(child => renderItemSync(child)).join('');
  } else if (typeof item === 'object') {
    return renderElementSync(item);
  } else {
    throw new TypeError(`Invalid render item type: ${typeof item}`);
  }
}

export { ASYNC_FALLBACK };
