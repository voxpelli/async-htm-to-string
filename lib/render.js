...
// linemod-add: import { renderItem, renderItemSync, ASYNC_FALLBACK } from './render-utils.mjs';
...
const renderToString = async (item) => { // linemod-prefix-with: export
  ...
  if (typeof item === 'object' && item !== null && !Array.isArray(item) && !isPromise(item) && /** @type {BasicRenderableElement<ElementProps>} */ (item).async === false) {
    try {
      ...
      const { ASYNC_FALLBACK } = await import('./render-utils.mjs'); // linemod-remove
      if (/** @type {Error} */ (err).message !== ASYNC_FALLBACK) throw err;
